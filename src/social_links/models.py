...
class SocialLink(TimestampMixin):
    """
    Represents a Social Media icon which will be displayed
    in the footer. Contains a link, name and image field.
    """

    name = models.CharField(max_length=255)
    url = models.URLField()
    icon = FilerImageField(related_name="links", on_delete=models.SET_NULL, null=True)
    order = models.PositiveIntegerField(
        default=0,
        help_text="Set this to prioritise the order of the icon, higher numbers are shown first.",
    )
    is_enabled = models.BooleanField(
        default=False, help_text="Check this if you would like to display this icon in the footer."
    )
